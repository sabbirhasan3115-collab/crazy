...
function periodBanner(){
  const now=new Date(),p=getActivePeriod(now.getHours(),now.getMinutes());
  if(!p)return '';
  return '<div class="period-banner active-'+p.type+'"><div class="dot"></div><span class="period-name">'+p.label+' in progress</span><span class="period-time">'+hhmm(p.sH,p.sM)+'&ndash;'+hhmm(p.eH,p.eM)+'</span></div>';}
...
function renderHeader(){
  ...
  var row=document.getElementById('headerStats'),cl=document.getElementById('liveClock'),tg=document.getElementById('soundToggle'),nt=document.getElementById('notifToggle'),ts=document.getElementById('testToggle');
  row.innerHTML=chips;row.appendChild(cl);row.appendChild(tg);row.appendChild(nt);row.appendChild(ts);}
...
function render(){renderHeader();renderDaily();renderCQ();renderAcc();}
